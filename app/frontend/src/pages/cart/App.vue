  ...
  <div id="app">
        ...
        <table class="table table-hover">
          <thead class="text-muted">
            <tr>
              ...
              <th class="d-none d-md-table-cell" scope="col">Цена</th>
              <th class="d-none d-md-table-cell" scope="col">Кол-во</th>
    ...
    >
      ...
      >
        ...
        <div class="card-body">
          <h4 class="card-title">В корзине пока пусто</h4>
...
export default {
  ...
  methods: {
    // Обработка события delete-click у cart-tr
    deleteClick(data) {
      this.cartCount = data.count;

      for (let ind in this.cartItems) {
        // Удаляем объект с данным id
        if (parseInt(this.cartItems[ind].id) == data.id) {
          this.cartItems.splice(ind, 1);
          break;
        }
      }
    },
    // Обработка события count-change у cart-tr
    countChange(data) {
      for (let ind in this.cartItems) {
        // Меняеи значение count у объекта с данным id
        if (parseInt(this.cartItems[ind].id) == data.id) {
          this.cartItems[ind].count = data.count.toString();
        }
      }
    },
  },
